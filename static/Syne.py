...
def ssh_execute_script(client_IP, client_name, client_password, remote_base_path, local_folder_path, local_bat_file):
    # SSH 세션 시작
    ...
    remote_file_to_check = 'C:\\informationSS3\\W1~82\\SOLu\\SeScore3.txt'
    while True:
        try:
    ...
    subdirectories = ['action', 'bad', 'good', 'log', 'SOLu']
    ...
    sftp.get(os.path.join(remote_base_path, 'report.txt'), os.path.join(local_folder_path, 'report.txt'))
...
with open(r'C:\Notion Project\SSH\SOLu\SOLu\AScore.txt', 'r', encoding='ANSI') as f:
    Ascore = int(f.read())
AscorePer = (round((Ascore/147)*100, 2))

with open(r'C:\Notion Project\SSH\SOLu\SOLu\SScore.txt', 'r', encoding='ANSI') as f:
    Sscore = int(f.read())
SscorePer = (round((Sscore/348)*100, 2))

with open(r'C:\Notion Project\SSH\SOLu\SOLu\PScore.txt', 'r', encoding='ANSI') as f:
    Pscore = int(f.read())
PscorePer = (round((Pscore/9)*100, 2))

with open(r'C:\Notion Project\SSH\SOLu\SOLu\LScore.txt', 'r', encoding='ANSI') as f:
    Lscore = int(f.read())
LscorePer = (round((Lscore/27)*100, 2))

with open(r'C:\Notion Project\SSH\SOLu\SOLu\SeScore.txt', 'r', encoding='ANSI') as f:
    Sescore = int(f.read())
...
plt.axis('equal')
# 그래프를 저장
plt.savefig(r'C:\Notion Project\Auto\Account_Chart.png')#이미지 저장
...
plt.axis('equal')
plt.savefig(r'C:\Notion Project\Auto\Service_Chart.png')
...
plt.axis('equal')
plt.savefig(r'C:\Notion Project\Auto\Patch_Chart.png')
...
plt.axis('equal')
plt.savefig(r'C:\Notion Project\Auto\Log_Chart.png')
...
plt.axis('equal')
plt.savefig(r'C:\Notion Project\Auto\Secure_Chart.png')

# Workbook이란 이름의 엑셀파일을 만듬
wb = openpyxl.Workbook()

# 파일 이름을 재설정
new_filename = r'C:\Notion Project\Auto\주요통신기반.xlsx'
...
img = Image(r'C:\Notion Project\Auto\Account_Chart.png')
img.width = 3*70  # 가로 크기 설정 (4 셀 = 4*64 픽셀)
img.height = 9*15  # 세로 크기 설정 (9 셀 = 9*15 픽셀)
w1.add_image(img, 'A2')  # A2 셀에 이미지 삽입

## 서비스 항목 이미지 ##
img2 = Image(r'C:\Notion Project\Auto\Service_Chart.png')
img2.width = 3*70
img2.height = 9*15
w1.add_image(img2, 'D2')
## 패치 항목 이미지 ##
img3 = Image(r'C:\Notion Project\Auto\Patch_Chart.png')
img3.width = 3*70
img3.height = 9*15
w1.add_image(img3, 'G2')
## 로그 항목 이미지 ##
img4 = Image(r'C:\Notion Project\Auto\Log_Chart.png')
img4.width = 4*70
img4.height = 14*15
w1.add_image(img4, 'A10')
## 보안 항목 이미지 ##
img5 = Image(r'C:\Notion Project\Auto\Secure_Chart.png')
img5.width = 4*70
img5.height = 14*15
...
wb=excel.Workbooks.open(r"C:\Notion Project\Auto\주요통신기반.xlsx") # 엑셀 파일을 읽어드려서 객체로 지정
ws_Report=wb.Worksheets("Report")
ws_Report.Select()
pdf_path=r"C:\Notion Project\Auto\주요통신기반.pdf"
